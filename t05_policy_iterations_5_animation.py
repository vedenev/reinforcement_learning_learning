...
fps = 0.5
fln_video_out = './policy_iterations_animation.mp4'
...
policy = np.zeros(U.shape, np.int64)
...
for iter_count in range(N_iters):
    ...
    for not_terminate_count in range(not_terminate_y.size):
        ...
        for_sum_vector = np.zeros(3, np.float32)
        dir_count = policy[yt, xt]
        ...
        for triplet_count in range(3):
            ...
            if movable[ytt, xtt]:
                for_sum_vector[triplet_count] += U_extended[ytt, xtt] * p_tt
            else:
                for_sum_vector[triplet_count] += U_extended[yt, xt] * p_tt
        sum_t = np.sum(for_sum_vector)
        U[yt, xt] = R[yt, xt] + gamma * sum_t
        ...
        max_ind_t = np.argmax(under_max_vector)
        max_t = under_max_vector[max_ind_t]
        if max_t > sum_t:
            policy[yt, xt] = max_ind_t
        optimal_dir_all[yt, xt] = max_ind_t
