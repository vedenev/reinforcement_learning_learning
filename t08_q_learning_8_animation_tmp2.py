...
for iter_count in range(N_iters):
    ...
    while True: # agent steps count
        ...
        if firts_step:
            ...
            if terminate[yt_old, xt_old]:
                Q[yt_old, xt_old, :] = R[yt_old, xt_old]
                break
            N[yt_old, xt_old, dir_t_old] += 1
            alfa_t = alfa(N[yt_old, xt_old, dir_t_old])
            #alfa_t = 1.0
            #alfa_t = 0.1
            Q[yt_old, xt_old, dir_t_old] = Q[yt_old, xt_old, dir_t_old] + alfa_t * (r_old + gamma * np.max(Q[yt, xt, :]) - Q[yt_old, xt_old, dir_t_old])
        
        U[yt, xt] = np.max(Q[yt, xt, :])
        U_extended[yt, xt] = U[yt, xt]
        
        
        dir_count = np.argmax(f(Q[yt, xt, :], N[yt, xt, :])) # algotrithm ansver for direction of the next step
        #dir_count = np.argmax(Q[yt, xt, :]) # algotrithm ansver for direction of the next step
