...
for iter_count in range(N_iters):
    ...
    while True: # agent steps count
        ...
        if firts_step:
            ...
            #alfa_t = 1.0
            #alfa_t = 0.1
            Q[yt_old, xt_old, dir_t_old] = Q[yt_old, xt_old, dir_t_old] + alfa_t * (r_old + gamma * Q_t_max - Q[yt_old, xt_old, dir_t_old])
        
        U[yt, xt] = np.max(Q[yt, xt, :])
        U_extended[yt, xt] = U[yt, xt]
        
        
        dir_count = np.argmax(f(under_max_vector, under_max_vector_N)) # algotrithm ansver for direction of the next step
